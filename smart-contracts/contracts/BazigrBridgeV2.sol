...
contract BazigrBridgeV2 is Ownable, ReentrancyGuard, Pausable {
    ...
    mapping(address => mapping(uint256 => bool)) public userProcessedNonces; // Track user-specific nonces

    // Bridge configuration
    string public constant SOURCE_CHAIN = "U2U";
    string public constant TARGET_CHAIN = "SEPOLIA";
    ...
    /**
     * @dev Lock tokens on source chain to bridge to target chain
    ...
    /**
     * @dev Unlock tokens on target chain after bridging from source chain
        ...
        whenNotPaused 
    {
        ...
        require(!userProcessedNonces[user][nonceToProcess], "User nonce already processed");
        require(bazigrToken.balanceOf(address(this)) >= amount, "Insufficient bridge balance");

        // Mark nonce as processed globally and for user
        processedNonces[nonceToProcess] = true;
        userProcessedNonces[user][nonceToProcess] = true;
        ...
        whenNotPaused 
    {
        require(amount > 0, "Amount must be greater than 0");
        require(!processedNonces[nonceToProcess], "Nonce already processed");
        require(!userProcessedNonces[msg.sender][nonceToProcess], "User nonce already processed");
        require(bazigrToken.balanceOf(address(this)) >= amount, "Insufficient bridge balance");

        // Mark nonce as processed globally and for user
        processedNonces[nonceToProcess] = true;
        userProcessedNonces[msg.sender][nonceToProcess] = true;
    ...
    /**
     * @dev Check if a user's nonce has been processed
     * @param user User address
     * @param nonceToCheck Nonce to check
     * @return True if user's nonce has been processed
     */
    function isUserNonceProcessed(address user, uint256 nonceToCheck) external view returns (bool) {
        return userProcessedNonces[user][nonceToCheck];
    }
